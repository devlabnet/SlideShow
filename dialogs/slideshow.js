...
SCRIPT_ADDGAL = "/ad-gallery/jquery.ad-gallery.min.js";
CSS_ADDGAL = "/ad-gallery/jquery.ad-gallery.css";
SCRIPT_FANCYBOX = '/fancybox2/jquery.fancybox.pack.js?v=2.1.5';
CSS_FANCYBOX = "/fancybox2/jquery.fancybox.css?v=2.1.5";
...
//				' src="' + 'data:text/html;charset=utf-8,' +  strVar + '"' +
	...
	function commitSlideShow(dialog) {
		//alert('commit : ' + this.id + " / dialog:" + dialog);
	...
	function loadValue() {
		...
		if (dialog.newSlideShowMode) {
			...
			//alert("Load New Value : " + this.type + " -> " + this.id + " -> " + this.getValue());
			dialog.slideshowDOM.setAttribute('data-'+this.id, this.getValue());
			switch ( this.type ) {
			case 'checkbox':
				break;
			...
			//alert("Load DOM Value : " + dialog.slideshowDOM.getAttribute('data-'+this.id));
...
//		str += "});";
	...
	function feedUlWithImages(dialog, ulObj) {
		for ( var i = 0; i < dialog.imagesList.length  ; i++ ) {
			...
			//if (isNaN(dialog.imagesList[i][IMG_PARAM.WIDTH])) alert('NaN 747');
